...
describe('Seguridad: Brute Force', () => {
  ...
  beforeEach((done) => {
    ...
    const params = [
      "admin",
      "$2a$10$yKEhxLcaY.aitra3nTLf3eOgjLxXYkcjCGyxBghZarce/NLvQfjxK", // "password"
      "admin@example.com"
    ];

    db.query(query, params, (err) => {
      if (err) {
        console.error("ERROR INSERTANDO ADMIN EN TEST:", err);
        return done(err); // Notificamos error a Jest
      }
      done(); // ✔ Listo, Jest continúa
    });
  });
